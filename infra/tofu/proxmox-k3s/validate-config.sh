...
if command -v tofu >/dev/null 2>&1; then
    ...
    success "OpenTofu found: $tofu_version"
elif command -v terraform >/dev/null 2>&1; then
    TOFU_TOOL="terraform"
    terraform_version=$(terraform version | head -1)
    success "Terraform found: $terraform_version"
else
    error "Neither OpenTofu nor Terraform found. Please install one of them."
    info "OpenTofu installation: https://opentofu.org/docs/intro/install/"
    info "Terraform installation: https://developer.hashicorp.com/terraform/downloads"
fi
...
if [ $ERRORS -eq 0 ]; then
    ...
    echo
    info "Next steps:"
    if [ "$TOFU_TOOL" = "tofu" ]; then
        echo "  1. Run: tofu init"
        echo "  2. Run: tofu plan"
        echo "  3. Run: tofu apply"
    elif [ "$TOFU_TOOL" = "terraform" ]; then
        echo "  1. Run: terraform init"
        echo "  2. Run: terraform plan"
        echo "  3. Run: terraform apply"
    else
        echo "  1. Install OpenTofu or Terraform"
        echo "  2. Run: <tool> init"
        echo "  3. Run: <tool> plan"
        echo "  4. Run: <tool> apply"
    fi
else
    echo
...
echo
echo "🚀 Ready for deployment with $TOFU_TOOL!"